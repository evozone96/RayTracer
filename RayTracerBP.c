...
void buildScene(void)
{
 ...
 //insertObject(o,&object_list);      // Insert into object list
  ...
  // Left wall
  o=newPlane(.05,.75,0.65,1,.55,.55,.75,1,1,0);  // Note the plane is highly-reflective (rs=rg=.75) so we
 ...
 // Front Wall
  o=newPlane(.05,.75,.65,1,.55,.55,.75,1,1,0);  // Note the plane is highly-reflective (rs=rg=.75) so we
  Scale(o,wallsize,wallsize,1);        // Do a few transforms...
  RotateY(o,-PI/2);
  RotateY(o,offset);
 Translate(o,-(wallsize * cos(offset)),-3,10 - (wallsize * sin(offset)));
 ...
 // Ceiling
  o=newPlane(.05,.75,.65,1,.55,.8,.75,1,1,0);  // Note the plane is highly-reflective (rs=rg=.75) so we
  Scale(o,wallsize,wallsize,1);        // Do a few transforms...
  RotateX(o,-PI/2);
  RotateY(o,offset);
 ...
 o=newPlane(.05,.75,.25,1,.55,.8,.75,1,1,0); 
 Scale(o,12,12,1);       
 RotateX(o,PI/2);
 RotateY(o,offset);
...
//~ currLight = light_list;
  ...
  //~ {    
   ...
   //~ //fprintf(stderr,"ambient diffuse spec %f %f %f\n", amdouble distToLighbient, diffuse, specular);
...
void rayTrace(struct ray3D *ray, int depth, struct colourRGB *col, struct object3D *Os)
{
 ...
 int main(int argc, char *argv[])
 {
  ...
  //        *interesting* scene.
  ///////////////////////////////////////////////////
  buildScene1();    // Create a scene. This defines all the
